...
def plot_vb_bandedges(ax, bandedges_data_file):
    ...
    hh = bandedges_data_file.variables["hh_bandedge"].value
    lh = bandedges_data_file.variables["lh_bandedge"].value
    so = bandedges_data_file.variables["ch_bandedge"].value
...
for folder, title_ending in zip(sweep_datafolder.folders, title_endings):
    fig, ax = plt.subplots()
    bandedges_file_path = folder.go_to('band_structure', 'BandEdges.dat')
