...
def minimize(
    input_file, variables_names, x0, target_filepath, target_variable, number=0, overwrite=True
):
    ...
    result = sp_minimize(
        ...
        x0,
        args=(input_file, variables_names, target_filepath, target_variable, number, overwrite),
        tol=1e-5,
    )
    return result
...
def get_target_value(
    input_file, target_filepath, target_variable, target_number=0, product="nextnano++"
):
...
def simple_optimize(
    ...
    for combination in all_combinations:
        ...
        target_val = post_func(
            get_target_value(input_file, target_filepath, target_variable, target_number)
        )
        if goal == "min":
...
def calculate_CV(output_directory_path, bias1=None, bias2=None, total=False, net_charge_sign=-1):
    ...
    dfile_hole = DataFile(
        dfolder.find("integrated_density_hole.dat", deep=True)[0],
        FirstVarIsCoordFlag=False,
        product="nextnano++",
    )
    dfile_electron = DataFile(
        dfolder.find("integrated_density_electron.dat", deep=True)[0],
        FirstVarIsCoordFlag=False,
        product="nextnano++",
    )

    bias_hole_set = set([var.name for var in dfile_hole.variables if var.name.endswith("_bias")])
    bias_electron_set = set(
        [var.name for var in dfile_electron.variables if var.name.endswith("_bias")]
    )
    ...
    if not common_biases_list:
        raise ValueError(
            "There is no common biases in integrated_density_hole.dat file and integrated_density_electron.dat file"
        )
    ...
    if bias1:
        ...
        if bias2:
            if bias2 not in common_biases_list:
                raise ValueError(
                    "Specified reference bias2 is not common bias for holes and electrons"
                )
            else:
    ...
    regions_hole_set = set(
        [var.name for var in dfile_hole.variables if not var.name.endswith("_bias")]
    )
    regions_electron_set = set(
        [var.name for var in dfile_electron.variables if not var.name.endswith("_bias")]
    )
    ...
    for region in common_regions_list:
        ...
        total_charge = (
            ...
            * elementary_charge
            * 1e6
            * (integrated_density_hole - integrated_density_electron)
        )
        c_list = []
